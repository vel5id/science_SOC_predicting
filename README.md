...
- **Canonical dataset build.** The published metrics were computed on the build with **1085 samples × 530 features, 81 fields, 20 farms** (years 2022–2023). `data/features/master_dataset.csv` must be *this* build. The dataset archive also contains later/experimental builds (e.g. with additional farms/years or a reduced feature set) — those will **not** reproduce the paper, and the loader will warn (wrong field count) or fail (missing feature columns) if you use one. `SpatialDataLoader` checks this on load.
