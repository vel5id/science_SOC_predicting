...
print("Starting to clean 2021 data...")
...
if os.path.exists(db_path):
    ...
    for table in tables:
        ...
        if 'year' in columns:
            cursor.execute(f"DELETE FROM {table} WHERE year = 2021;")
            print(f"  Deleted 2021 records from table: {table}")
        ...
        for file in glob.glob(f"{folder}/*.*"):
            ...
            try:
                ...
                if 'year' in df.columns:
                    ...
                    if n_before != n_after:
                        print(f"    Removed {n_before - n_after} rows for 2021 in {file}.")
                        if file.endswith(".csv"):
