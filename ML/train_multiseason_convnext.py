...
# ==========================================
# 2. MULTISPECTRAL CONVNEXT / SE-CNN
# ==========================================
...
class ConvNeXtBlock(nn.Module):
    def __init__(self, dim, drop_path=0.):
        ...
        self.pwconv1 = nn.Linear(dim, 4 * dim) # pointwise/1x1 convs, implemented with linear
        self.act = nn.GELU()
    ...
    def forward(self, x):
        ...
        x = self.se(x) # Add Squeeze-and-Excitation for multi-spectral weighting
        x = input + x
        return x

class MultiSpectralConvNeXt(nn.Module):
    def __init__(self, in_channels, out_features=1):
        """Architecture designed to preserve channel information for multi-spectral tabular-like patches"""
        super().__init__()
        # Initial projection explicitly preserving/expanding channels rather than squashing
        # Allows learning non-linear feature interactions per-pixel immediately
        stem_dim = 128
        self.stem = nn.Sequential(
            nn.Conv2d(in_channels, stem_dim, kernel_size=1, stride=1),
        ...
        self.stages = nn.Sequential(
            ...
            ConvNeXtBlock(stem_dim),
            nn.AvgPool2d(2, stride=2), # 32x32 -> 16x16
            
            nn.Conv2d(stem_dim, 256, kernel_size=1),
            ConvNeXtBlock(256),
            ConvNeXtBlock(256),
            nn.AvgPool2d(2, stride=2), # 16x16 -> 8x8
...
def run_multiseason_ablation(df, patch_cache, base_bands_idx, engineered_indices, 
    ...
    MODELS_DIR = "ML/ml_models_convnext"
    ...
    for patch_size in patch_sizes:
        ...
        for step in range(len(engineered_indices) + 1):
            ...
            model = MultiSpectralConvNeXt(in_channels=in_c).to(device)
    ...
    csv_path = f'ML/results/convnext_ablation_{target_col}.csv'
...
if __name__ == '__main__':
    ...
    for t in targets:
        ...
        # Save incrementally
        global_csv_path = 'ML/results/convnext_ablation_summary_all.csv'
